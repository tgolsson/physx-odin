...
struct PodStructGen {
    ...
    void begin_struct(const char* cname) {
        fprintf(cppfile, "struct %s {\n", cname);
        fputs("{\n", definitions_file);
		current_indent += 4;
        indent();
        fprintf(definitions_file, R"("name": "%s",)", cname);
		indent();
        fprintf(definitions_file, R"("fields": [\n)");
		current_indent += 4;
        pos = 0;
        padIdx = 0;
    }
    ...
    void add_field(
        ...
        fprintf(cppfile, "    %s %s;\n", cppType, cppName);
		indent();
        fprintf(definitions_file, "{\"name\": \"%s\", \"type\": %s, \"offset\": %u, \"size\": %u},\n", cppName, cppType, offset, size);
        pos += size;
    }
